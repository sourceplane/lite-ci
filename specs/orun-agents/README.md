...
| AG1 | Authoring + seal/pull + catalog: `agents/*.md` → `AgentTypeSnapshot` (persona → body blob, frontmatter → envelope); `orun agent import`, `orun agent pull <type>@<hash>` (set-difference via `objremote`); **catalog projection** (`AgentType` entity + edges: owner, `mayAffect`) | ✅ Shipped (local seal `import` + read-back `show`; networked pull rides the WP remote slice) |
| AG2 | The delegation runtime: `AgentDriver` seam (Claude Code first, headless stream-JSON); brief assembler (agent-type + task contract + `catalog affected` + base literacy, sealed); MCP manager + tool policy; append-only session event log; `orun agent run` (headless one-shot) | ✅ Shipped (driver seam + stub driver + brief assembler + tool policy + session log + `orun agent run [--dry-run]`; Claude Code driver + live MCP wiring ride AG4/WP5) |
| AG3 | TUI **Agent mode**: a 6th `internal/tui` mode (`3`) — git-authored agent types (from the AG1 catalog projection) with detail (harness/model/owner/autonomy/mayAffect/persona) + a streaming session transcript pane; fully local, no cloud | ✅ Shipped (navigable surface + streaming transcript; in-TUI launch trigger pairs with AG4) |
| AG4 | Driver conformance + session seal: the `AgentDriver` protocol + a conformance oracle (a second driver passes the full lifecycle unchanged); session seal → `AgentSessionSnapshot` + `orun agent replay <session>@<hash>` | ✅ Shipped |
