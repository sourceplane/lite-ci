...
| `ORUN_WORKSPACE` | Workspace scope for remote state — a Workspace ID `ws_…`, slug, or `org_…`; overrides the linked workspace and the workspace selected with [`orun workspace use`](../cli/orun-workspace.md). Equivalent to `--workspace` |
