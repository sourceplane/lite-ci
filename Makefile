...
test-state-redesign:
	...
	@echo "🧪 Unified MCP: transport (orun-mcp UM0) + pen provider + platform provider (UM1-UM2)"
	@go test -count=1 -race ./internal/mcpserve/...
	@go test -count=1 -race ./internal/penmcp/...
	@go test -count=1 -race ./internal/platformmcp/...
...
test-object-model:
	...
	@echo "🧪 object-model: end-to-end walk + dedup/disk-win (test-plan.md §4, §6)"
	@go test -count=1 -race ./internal/objmodele2e/...
